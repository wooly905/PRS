...
PRS は、SQL Server データベースと開発者（または AI エージェント）の間のギャップを埋めるために設計された強力なツールです。データベーススキーマをローカルの人間が読める形式の Markdown ファイルにダンプすることができます。これらのスキーマは、豊富なコマンドラインインターフェース（CLI）を介して照会できます。
...
PRS には、Claude に CLI を使用してデータベーススキーマを効果的にクエリする方法を教える Claude Code スキルが付属しています。
...
このスキルは、Claude がアクティブスキーマの確認、DDL フォーマットによるスキーマ理解、外部キー関係の追跡、実際のスキーマ定義に基づいた SQL クエリの生成を行うよう導きます。
