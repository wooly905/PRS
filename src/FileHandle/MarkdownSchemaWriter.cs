...
internal class MarkdownSchemaWriter : ISchemaWriter
{
    ...
    private void BuildMarkdownContent()
    {
        ...
        {
            var tableKey = $"{table.TableSchema}.{table.TableName}";
    ...
    {
        // Table header
        _content.AppendLine($"### {table.TableSchema}.{table.TableName}");
    ...
    private void WriteColumn(ColumnModel column)
    {
        ...
        _content.Append($"  - {column.ColumnName} ({dataType}, {nullable}, Position: {position})");
